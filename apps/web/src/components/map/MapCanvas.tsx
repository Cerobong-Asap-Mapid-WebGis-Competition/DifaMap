...
export default function MapCanvas({
  ...
  useEffect(() => {
    ...
    // LAPORAN KOMUNITAS
    //
    // Laporan yang dikirim lewat "Bagikan Laporan" tidak selalu membentuk titik
    // baru: bila ada titik survei dalam radius 80 meter, server menempelkannya
    // ke situ. Itu benar - dua orang melaporkan trotoar yang sama tidak boleh
    // menjadi dua pin - tetapi akibatnya laporan yang menempel jadi tak terlihat
    // sama sekali di peta, dan pelapornya menyimpulkan laporannya hilang.
    //
    // Yang digambar di sini hanya laporan yang letaknya BERBEDA dari titik
    // induknya. Ukurannya bukan tebakan: sembilan puluh empat aktivitas hasil
    // impor MAPID semuanya berjarak di bawah satu meter dari lokasinya, jadi
    // ambang dua puluh lima meter tidak akan pernah menggambar ulang titik yang
    // pinnya sudah ada.
    // =========================================================================
    if (currentMode === 'NONE') {
      const AMBANG_TERPISAH = 25;

      const laporanKomunitas = activities.filter((act: any) => {
        if (typeof act?.latitude !== 'number') return false;

        const induk = locations.find((l: any) => l.id === act.locationId);
        if (!induk) return true;

        const jarak =
          Math.hypot(act.latitude - induk.latitude, act.longitude - induk.longitude) * 111320;

        return jarak > AMBANG_TERPISAH;
      });

      laporanKomunitas.forEach((act: any) => {
        const el = document.createElement('div');
        el.className = 'laporan-pin-wrapper';

        const judul = String(act.title ?? 'Laporan komunitas').replace(/"/g, '&quot;');
        const skor = typeof act.aiScore === 'number' ? act.aiScore.toFixed(1) : null;
        ...
        el.innerHTML = `
          <div style="
            ...
            transition: transform 0.15s ease;
            filter: drop-shadow(0 2px 5px rgba(0,0,0,0.3));
          " onmouseenter="this.style.transform='scale(1.2) translateY(-2px)'" onmouseleave="this.style.transform='scale(1) translateY(0)'"
             title="Laporan komunitas: ${judul}${skor ? ` (skor ${skor})` : ''}">
            <svg width="30" height="40" viewBox="0 0 24 32" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M12 0C5.373 0 0 5.373 0 12C0 20.5 10.5 30.75 11.08 31.33C11.58 31.83 12.42 31.83 12.92 31.33C13.5 30.75 24 20.5 24 12C24 5.373 18.627 0 12 0Z" fill="#F59E0B"/>
              <circle cx="12" cy="11.5" r="8.2" fill="#FFFFFF"/>
            </svg>
            <span style="position:absolute;top:6px;left:0;right:0;display:flex;justify-content:center;pointer-events:none;">
              ${ikonSvg('PEDESTRIAN_PATH', 15, '#F59E0B')}
            </span>
          </div>
        `;
